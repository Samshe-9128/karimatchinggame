...
export default function useMemoryGame() {
  ...
  function startFreshRound(existingSession) {
    ...
    previewTimerRef.current = setTimeout(() => {
      if (sessionRef.current !== existingSession) {
        return;
      }
  ...
  function useHint() {
    ...
    if (hintsRef.current === null || hintsRef.current <= 0) {
      return;
    }

    if (previewing || checking || blackout || !activeRef.current) {
      return;
    }
  ...
  function handleCardClick(cardId) {
    if (!activeRef.current || previewing || checking || blackout) {
      return;
    }

    if (hinting.includes(cardId)) {
      return;
    }

    if (flippedRef.current.includes(cardId)) {
      return;
    }

    if (matchedRef.current.has(cardId)) {
      return;
    }
  ...
  function goToNextLevel() {
    if (travelLockRef.current || level >= MAX_LEVEL) {
      return;
    }
